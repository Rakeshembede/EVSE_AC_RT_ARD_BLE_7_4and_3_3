...
bool RTCx::autoprobe(const uint8_t *addressList, uint8_t len)
{
	for (uint8_t i = 0; i < len; ++i) {
		...
		Wire1.beginTransmission(addressList[i]);
		Wire1.write(uint8_t(0));
		Wire1.endTransmission();

		// Try reading a byte
		Wire1.requestFrom(addressList[i], (uint8_t)1);
		if (Wire1.available()) {
...
void RTCx::init(void) const
{
	if (device == PCF85263) {
		...
		Wire1.write((uint8_t)0x23); // Start register
		Wire1.write((uint8_t)0x00); // 0x23
		Wire1.write((uint8_t)0x00); // 0x24 Two's complement offset value
		Wire1.write((uint8_t)0x12); // 0x25 Normal offset correction, enable low-jitter mode, set load caps to 12.5pF
		Wire1.write((uint8_t)0x00); // 0x26 Battery switch reg, same as after a reset
		Wire1.write((uint8_t)0x00); // 0x27 Enable CLK pin, using bits set in reg 0x28
		Wire1.write((uint8_t)0x07); // 0x28 Realtime clock mode, no periodic interrupts, CLK pin off
		Wire1.write((uint8_t)0x00); // 0x29
		Wire1.write((uint8_t)0x00); // 0x2a
		Wire1.endTransmission();
	}
	else {
		enableBatteryBackup();
...
bool RTCx::readClock(struct tm *tm, timeFunc_t func) const
{
	...
	while (true) {
		...
		tm->tm_sec = bcdToDec(Wire1.read() & 0x7f);
		tm->tm_min = bcdToDec(Wire1.read() & 0x7f);
		...
		if (device == PCF85263) {
			...
			tm->tm_mday = bcdToDec(Wire1.read() & 0x3f);
			tm->tm_wday = (Wire1.read() & 0x07); // Clock uses [0..6]
		}
		else {
			tm->tm_wday = (Wire1.read() & 0x07) - 1; // Clock uses [1..7]
			tm->tm_mday = bcdToDec(Wire1.read() & 0x3f);
		}

		tm->tm_mon = bcdToDec(Wire1.read() & 0x1f) - 1; // Clock uses [1..12]
		if (sz >= 7)
			tm->tm_year = bcdToDec(Wire1.read()) + 100; // Assume 21st century
		else
...
{
	// Find which register to read from
	Serial.println(F("***** got called here*****"));
	uint8_t sz = 0;
	uint8_t reg = getRegister(func, sz);
	...
	if (func == TIME) {
		if (device == PCF85263) {
			...
			Wire1.beginTransmission(address);
			Wire1.write((uint8_t)0x2e);
			Wire1.write((uint8_t)1); // 0x2e Stop the clock
			Wire1.write((uint8_t)0xa4); // 0x2f STOP
			// Register wraps round to 0x00
			Wire1.write((uint8_t)0); // Clear hundredths of seconds
			...
			if (device == MCP7941x)
				...
				Serial.println("The year to set is :" + tm->tm_year);
	...
	// Wire1.beginTransmission(address);
	// Wire1.write(reg);
	Wire1.write(decToBcd(tm->tm_sec) | clockHalt);

	Wire1.write(decToBcd(tm->tm_min));
	Wire1.write(decToBcd(tm->tm_hour)); // Forces 24h mode

	if (device == PCF85263) {
		// Day of month and weekday are in reverse order compared to
		// DS1307 and MCP7941x
		Wire1.write(decToBcd(tm->tm_mday));
		Wire1.write(decToBcd(tm->tm_wday)); // Clock uses [0..6]
	}
	else {
		Wire1.write(decToBcd(tm->tm_wday + 1) | osconEtc); // Clock uses [1..7]
		Wire1.write(decToBcd(tm->tm_mday));
	}

	// Leap year bit on MCP7941x is read-only so ignore it
	Wire1.write(decToBcd(tm->tm_mon + 1));

	if (sz >= 7)
		Wire1.write(decToBcd(tm->tm_year % 100));
...
bool RTCx::readTimeSaver(struct tm *tm, uint8_t reg, uint8_t sz) const
{
	...
	tm->tm_sec = 0;
	tm->tm_min = bcdToDec(Wire1.read() & 0x7f);
	tm->tm_hour = bcdToDec(Wire1.read() & 0x3f);
	tm->tm_wday = 0;
	tm->tm_mday = bcdToDec(Wire1.read() & 0x3f);
	uint8_t wdayMonth = Wire1.read();
