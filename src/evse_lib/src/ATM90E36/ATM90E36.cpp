...
unsigned short ATM90E36::CommEnergyIC(unsigned char RW, unsigned short address, unsigned short val) 
{
  ...
  delayMicroseconds(10);
   ...
   digitalWrite (4, HIGH);//@test for isolater
  ...
  delayMicroseconds(10);
  digitalWrite (4, LOW); //@test for isolater
#if !defined(ENERGIA)
  SPI.endTransaction();
#endif
...
{  
  ...
  /* Enable SPI */
 // SPI.begin();
  /* 14 - sck,  12 - miso, 13 - mosi, 5(_energy_CS) - ss   */ 
  SPI.begin(14, 12, 13, _energy_CS);  
...
void ATM90E36:: config_registers()
{
 #endif
