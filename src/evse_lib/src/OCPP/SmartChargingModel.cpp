...
int ChargingSchedulePeriod::getNumberPhases(){
  Serial.print("[SmartChargingModel] Unsupported operation: ChargingSchedulePeriod::getNumberPhases(); No phase control implemented");
...
bool ChargingSchedule::inferenceLimit(time_t t, time_t startOfCharging, float *limit, time_t *nextChange) {
  ...
  *nextChange = INFINITY_THLD; //defaulted to Infinity
  switch (chargingProfileKind) {
    case (ChargingProfileKindType::Absolute):
      ...
      if (startSchedule > 0) {
        ...
        Serial.print("[SmartChargingModel] Undefined behaviour: Inferencing limit from absolute profile, but neither startSchedule, nor start of charging are set! Abort\n");
        return false;
      }
      break;
    case (ChargingProfileKindType::Recurring):
      if (recurrencyKind == RecurrencyKindType::Daily) {
        ...
        *nextChange = basis + SECS_PER_WEEK;
        ...
        Serial.print("[SmartChargingModel] Undefined behaviour: Recurring ChargingProfile but no RecurrencyKindType set! Assume \"Daily\"");
        basis = t - ((t - startSchedule) % SECS_PER_DAY);
  ...
  if (t < basis) { //check for error
    Serial.print("[SmartChargingModel] Error in SchmartChargingModel::inferenceLimit: time basis is smaller than t, but t must be >= basis\n");
    return false;
  }
...
ChargingProfile::ChargingProfile(JsonObject *json){
  ...
  if (DEBUG_OUT) {
    Serial.print("[SmartChargingModel] ChargingProfile created with chargingProfileId = ");
  ...
  if (DEBUG_OUT) {
    Serial.print("[SmartChargingModel] chargingProfilePurposeStr=");
  ...
  if (DEBUG_OUT) {
    Serial.print("[SmartChargingModel] recurrencyKindStr=");
  ...
  if (!getTimeFromJsonDateString(validFromStr, &validFrom)){
    //non-success
    Serial.print("[SmartChargingModel] Error reading validFrom. Expect format like 2020-02-01T20:53:32.486Z. Assume year 1970 for now\n");
  }

  const char *validToStr = (*json)["validTo"] | INFINITY_STRING;
  if (!getTimeFromJsonDateString(validToStr, &validTo)){
    //non-success
    Serial.print("[SmartChargingModel] Error reading validTo. Expect format like 2020-02-01T20:53:32.486Z. Assume year 2037 for now\n");
  }
