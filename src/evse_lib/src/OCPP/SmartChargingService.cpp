...
void SmartChargingService::loop(){
  ...
  if (now() >= nextChange){
    ...
    if (DEBUG_OUT) Serial.print("[SmartChargingService] New Limit! Values: {scheduled at = ");
    if (DEBUG_OUT) printTime(nextChange);
    if (DEBUG_OUT) Serial.print(", nextChange = ");
...
void SmartChargingService::writeOutCompositeSchedule(JsonObject *json){
  Serial.print("[SmartChargingService] Unsupported Operation: SmartChargingService::writeOutCompositeSchedule\n");
}

void SmartChargingService::beginCharging(time_t t, int transactionID){
  if (chargingSessionIsActive) {
    Serial.print("[SmartChargingService] Error: begin new Charging session though there is already running one!\n");
  }
...
void SmartChargingService::endChargingNow(){
  if (!chargingSessionIsActive) {
    Serial.print("[SmartChargingService] Error: end Charging session but there isn't running one!");
  }
...
void SmartChargingService::updateChargingProfile(JsonObject *json){
  ...
  if (DEBUG_OUT) {
    Serial.print("[SmartChargingService] Charging Profile internal model\n");
    chargingProfile->printProfile();
  }

  int stackLevel = chargingProfile->getStackLevel();
  if (stackLevel >= CHARGEPROFILEMAXSTACKLEVEL) {
    Serial.print("[SmartChargingService] Error: Stacklevel of Charging Profile is greater than CHARGEPROFILEMAXSTACKLEVEL\n");
