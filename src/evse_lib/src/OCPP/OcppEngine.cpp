...
boolean processWebSocketEvent(uint8_t *payload, size_t length)
{
  ...
  Serial.print("IN process websocket ");
...
void handleReqMessage(JsonDocument *json)
{
  ...
  receivedOcppOperations.add(req);
  ;                      // enqueue so loop() plans conf sending
...
ChargePointStatusService *getChargePointStatusService()
{
  ...
  {
    Serial.print("[OcppEngine] Error: in OcppEngine, there is no chargePointStatusService set, but it is accessed!\n");
    // no error catch
  }
