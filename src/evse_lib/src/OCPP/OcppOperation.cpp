...
// extern TinyGsm ota_modem;
// extern TinyGsmClient ota_client;
...
extern bool sendTXTGsmStr(String &payload);
extern bool sendTXTGsm(uint8_t *payload, size_t length, bool headerToPayload);
extern bool sendPingGsmStr(String &payload);
extern bool sendPingGsm(uint8_t *payload, size_t length, bool headerToPayload);
extern uint8_t createHeader(uint8_t *headerPtr, WSopcode_t opcode, size_t length, bool mask, uint8_t maskKey[4], bool fin);
extern bool sendFrame(WSopcode_t opcode, uint8_t *payload, size_t length, bool fin, bool headerToPayload);
...
boolean OcppOperation::sendReq(){
  ...
  if (timeout_active && millis() - timeout_start >= TIMEOUT_CANCEL){
    ...
    Serial.println("Boot_Notification:"+String(Boot_Notification_Cmp));
